...
2. **Sol Advisor** is the primary design reference for `agent-config` (inspect Host first; remain host-neutral at the Skill layer; treat single-model and multi-model peer execution as first-class modes; use user-confirmed model-tier Profiles instead of guessing model intelligence; resolve abstract effort policies to actual Host-supported values; use optional companion MCP for persistence and Host mutation while remaining usable in plan-only mode; do not own formal ticket decomposition, implementation, review convergence, or final acceptance; require explicit approval before configuration mutation).
