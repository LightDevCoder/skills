...
- **Original Skill path:** `skills/productivity/teach` (contains `SKILL.md`, `agents/openai.yaml`, `GLOSSARY-FORMAT.md`, `LEARNING-RECORD-FORMAT.md`, `MISSION-FORMAT.md`, `RESOURCES-FORMAT.md`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/productivity/teach` to Light `skills/teach/`.
  2. Kept `SKILL.md` verbatim (no bloat, no rewrite). Supporting files kept verbatim (`GLOSSARY-FORMAT.md`, `LEARNING-RECORD-FORMAT.md`, `MISSION-FORMAT.md`, `RESOURCES-FORMAT.md` remain reference-only supporting docs at the package root).
...
Verification: `SKILL.md` frontmatter `name: teach` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
