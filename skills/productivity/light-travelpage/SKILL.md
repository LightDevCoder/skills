...
Verify the actual hosted URL, authentication on the page, JSON, ticket assets and API, then two independent sessions: shared member/settings/bill/task/ticket changes, conflict response, failed-save recovery and idempotent retry. Chromium and WebKit can block PDF `iframe` content in a modal: in a narrow mobile viewport, confirm that the ticket PNG is actually visible inside `#ticket-dialog`, zoom works, and the original PDF link remains available. Also check both languages, complete/pending flight and stay cards, itinerary, readable regional navigation and saved expense after refresh. Switching language and polling must not erase a form draft. Run focused changed-feature tests during updates rather than repeat unrelated checks.
