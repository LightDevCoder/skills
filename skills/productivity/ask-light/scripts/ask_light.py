...
# skills/review/project-review (see its references/WORKFLOW.md). The `.review-loop/`
...
def _has_collection_package(root: Path, name: str) -> bool:
    return (root / name / "SKILL.md").is_file() or any(
        root.glob(f"*/{name}/SKILL.md")
    )
...
def _collection_root_candidates(cwd: Path | None = None) -> list[Path]:
    ...
    for base in [start, *list(start.parents)[:5]]:
        root = base / "skills"
        if root.is_dir() and _has_collection_package(root, "ask-light") and _has_collection_package(root, "socratic"):
            candidates.append(root)
    # A source checkout adds one category level; an installed package is flat.
    for script_collection in list(Path(__file__).resolve().parents)[2:4]:
        if _has_collection_package(script_collection, "ask-light") and _has_collection_package(script_collection, "socratic"):
