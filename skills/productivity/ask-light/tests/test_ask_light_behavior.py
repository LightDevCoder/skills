...
def install_host_fixture_skills(root: Path) -> list[dict[str, object]]:
    ...
    for entry in ASK_LIGHT.load_map()["skills"]:
        fields, error = ASK_LIGHT.read_frontmatter(next(ROOT.parents[1].glob(f'*/{entry["name"]}/SKILL.md')))
...
class DiscoveryAndProvenanceMatrixTest(unittest.TestCase):
    ...
    def test_source_checkout_root_discovery(self) -> None:
        """SPEC §21: Discover roots from source checkout containing skills/productivity/ask-light and skills/socratic."""
    ...
    def test_categorized_source_checkout_discovery(self) -> None:
        repo_root = self.root / "categorized"
        skills_dir = repo_root / "skills"
        write_skill(skills_dir / "productivity", "ask-light")
        write_skill(skills_dir / "thinking", "socratic")
        roots = ASK_LIGHT.discover_roots(cwd=repo_root)
        self.assertIn(str(skills_dir.resolve()), [r["path"] for r in roots])
