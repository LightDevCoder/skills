...
Resolution priority is a local preference for the region, an allowed place-specific provider, then the regional default. Reset removes only that region's preference. In a provider, use a supported verified URL first, then a supported provider ID, compatible sourced coordinates, or original-name/address search. Amap coordinates require GCJ02; other implemented coordinate links require WGS84. No coordinate conversion is performed. Unknown or incompatible systems fall back to address search. Preserve legacy Google fields; supported URLs are used only for the Google choice. Unsupported links fall back to search without deleting source fields.
