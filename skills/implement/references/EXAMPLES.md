...
2. `implement` recognizes that routing would materially help and offers the user
   a choice:
   - A. Use `agent-config` to plan multi-agent roles and execution waves.
   - B. Continue directly with single-agent execution using the current model.
3. User chooses option A (accepts).
4. `implement` invokes `agent-config` with the bounded units and host evidence.
   `agent-config` returns a role-clear plan with waves and ownership.
