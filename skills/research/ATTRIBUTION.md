...
- **Original Skill path:** `skills/engineering/research` (contains `SKILL.md`, `agents/openai.yaml`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/engineering/research` to Light `skills/research/`.
...
Verification: `SKILL.md` frontmatter `name: research` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
