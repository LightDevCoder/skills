...
- **Original Skill path:** `skills/engineering/prototype` (contains `SKILL.md`, `agents/openai.yaml`, `LOGIC.md`, `UI.md`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/engineering/prototype` to Light `skills/prototype/`.
  2. Kept `SKILL.md` verbatim (no bloat, no rewrite). Supporting files kept verbatim (`LOGIC.md`, `UI.md` remain reference-only supporting docs).
...
Verification: `SKILL.md` frontmatter `name: prototype` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
