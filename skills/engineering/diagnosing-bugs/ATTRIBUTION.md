...
- **Original Skill path:** `skills/engineering/diagnosing-bugs` (contains `SKILL.md`, `agents/openai.yaml`, `scripts/hitl-loop.template.sh`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/engineering/diagnosing-bugs` to Light `skills/diagnosing-bugs/`.
  2. Kept `SKILL.md` verbatim (no bloat, no rewrite). Supporting files kept verbatim (`scripts/hitl-loop.template.sh` remains executable and at the same relative `scripts/` path).
...
Verification: `SKILL.md` frontmatter `name: diagnosing-bugs` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
