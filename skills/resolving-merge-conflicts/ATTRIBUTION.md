# Attribution — resolving-merge-conflicts
...
- **Original Skill path:** `skills/engineering/resolving-merge-conflicts` (contains `SKILL.md`, `agents/openai.yaml`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/engineering/resolving-merge-conflicts` to Light `skills/resolving-merge-conflicts/`.
...
Verification: `SKILL.md` frontmatter `name: resolving-merge-conflicts` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
