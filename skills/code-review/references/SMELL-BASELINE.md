# Smell baseline — Standards axis floor

Fixed set of Fowler code smells (_Refactoring_, ch. 3) that the Standards axis
always carries underneath whatever the repository documents. Each smell is a
labelled heuristic with a suggested move, not a hard violation. Repo standards
override this baseline where they endorse a flagged pattern; tooling-enforced
style is always skipped.
...
- **Mysterious Name** — a function, variable, or type whose name does not
  ...
  → rename it; if no honest name comes, the surrounding design is murky.

- **Duplicated Code** — the same logic shape appears in more than one hunk or
  file in the change.
  → extract the shared shape, call it from both places.
...
- **Primitive Obsession** — a primitive or string standing in for a domain
  ...
  → give the concept its own small type or value object.
...
- **Refused Bequest** — a subclass or implementer that ignores or overrides
  most of what it inherits.
  → drop the inheritance, use composition.

Use the prefix `possible` when reporting (e.g., `possible Feature Envy: …`),
quote the hunk being flagged, and note the suggested move rather than
