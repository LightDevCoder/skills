# Attribution — writing-for-agents
...
- **Original Skill path:** `skills/productivity/writing-for-agents` (contains `SKILL.md`, `agents/openai.yaml`, `SKILL-MECHANICS.md`)
...
- **Transformation summary:** PORT — NO REDESIGN (SPEC §14/§16). Preserved mature behavior verbatim; only the following Light integration changes were applied:
  1. Flattened path from upstream `skills/productivity/writing-for-agents` to Light `skills/writing-for-agents/`.
  2. Kept `SKILL.md` verbatim (no bloat, no rewrite). Supporting files kept verbatim (`SKILL-MECHANICS.md` remains a sibling reference to `SKILL.md`).
...
Verification: `SKILL.md` frontmatter `name: writing-for-agents` matches, `agents/openai.yaml` is valid, and aside from this attribution no `mattpocock/skills` runtime coupling remains in executable content.
