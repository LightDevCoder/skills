...
- **Strict scope and validation safety:** Added strict scope vocabulary enforcement (`current-workflow`, `independent`, `standalone`), explicit `Skill: none` validation preventing hard constraint bypass, and scope preservation across explicit user approval.
- **Trusted host capability evidence:** Defined trusted host evidence schema (`approvedUserInvokedTransition: {"state": "available", "source": "host-runtime"}`) for approved user-invoked transitions, rejecting model inference and unverified booleans.
