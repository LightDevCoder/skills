...
- **ask_light.py evidence service:** The Python helper is now purely an evidence, catalog, recipe, and validation service (`ask-light-evidence/1`). Removed `PROJECT_STATE_INTENT_PATTERN` and all regex/taskKind/score-based semantic routing authority. Returns structured facts (projectContract, currentEffort, spec, tickets, review, artifactSignals) and scoped hard constraints without any deterministic Skill recommendation.
- **Post-model selection validation:** Added deterministic `validate_recommendation` (`--mode validate`) that verifies map membership, first-party single copy, frontmatter validity, host availability, invocation compatibility, local pointer resolution, and scoped constraint compliance without ever substituting another Skill.
- **Scoped hard constraints:** Hard project state (uninitialized, ambiguous effort, active review, stale review) is exposed as scoped constraints (`appliesTo: current-workflow`) rather than global early returns, preventing project state from hijacking independent tasks, new efforts, or standalone requests.
- **Ticket frontier and clarification signals:** Enhanced ticket parsing into `ready`, `blocked`, `claimed`, `resolved`, and `unknown` buckets using `Status:` and `Blocked by:` contracts. Replaced filename-based clarification guessing with content-validated handoff signal classification.
- **Host-aware approval transition:** Added revalidation before execution and host capability awareness for approved transitions into user-invoked targets.
...
- **ask-light contracts:** Updated `SKILL.md`, `discovery-contract.md`, and `light-skill-map.json` to reflect the model-led advisor contract and candidate-hints-only map authority.

### Added

- **Comprehensive test suite:** Deterministic evidence tests, router boundary tests (proving Python makes no semantic choice), hard-state scoping tests, review transaction and freshness regression suites, and `agent-config`/`implement` relationship tests.
