...
Status: `TAG PENDING` at the time of this commit — the receipt is completed
after the `v0.1.4` tag is published and the fresh-install verification
against the published tag is recorded in
...
| Release | `v0.1.4` |
| Release commit | the `v0.1.4` tag on main |
| Release URL | https://github.com/LightDevCoder/skills/releases/tag/v0.1.4 |
| Scope | New first-party `light-kanban-worker` Skill (full admission path), scheduled Light-Kanban worker workflow, version/documentation synchronization cleanup, installation verification |
...
| Full test suite PASS | PASS locally (90 collection assertions, 19 package suites); CI on main |
| Catalog synced | PASS |
| Installation docs synced | PASS |
| Fresh installation against the published v0.1.4 tag | PENDING — recorded in [INSTALLATION_VERIFICATION.md](INSTALLATION_VERIFICATION.md) |
