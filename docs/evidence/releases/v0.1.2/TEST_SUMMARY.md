...
| collection discovery | `powershell -File tests/collection-discovery-tests.ps1` | `PASS` — 951 assertions, including Skill-guide, workflow, release-evidence, and bilingual semantic-pair parity. |
...
| Tagged fresh install | `npx skills add LightDevCoder/skills#v0.1.2 ...` plus `npx skills list` | `NOT TESTED` until the release tag exists; see [installation evidence](INSTALLATION_VERIFICATION.md). |
| Generic `latest` fresh install | `npx skills add LightDevCoder/skills ...` plus `npx skills list` | `NOT TESTED` until the release tag exists; see [installation evidence](INSTALLATION_VERIFICATION.md). |
| Release-commit CI | GitHub Actions `collection-quality` | `NOT TESTED` until the workflow has run on the release commit. |
