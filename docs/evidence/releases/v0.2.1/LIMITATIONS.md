...
4. **TypeSafe Jev API Key Requirement:** Optional semantic acceleration in `ask-light` and `agent-config` requires `TYPESAFE_API_KEY` and the `typesafe-sdk` package. When unavailable, both skills automatically fall back to deterministic baselines without functional disruption or security compromise.
