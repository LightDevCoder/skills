...
- **TypeSafe Jev System One Semantic Acceleration:** Integrated optional fast semantic judgments into `ask-light` (bounded candidate Choice, execution intent Noul calibration, ambiguity detection, readiness Score), `agent-config` (abstract task profiling across routine, standard, high complexity tiers without vendor model coupling), and `project-init` (interactive repository Jev onboarding gate, .gitignore protection, global skill reuse, and stack-aware guidance), maintaining deterministic fail-closed safety and zero-dependency offline fallback.
