...
- **TypeSafe Jev System One Semantic Acceleration:** Integrated optional fast semantic judgments into `ask-light` (bounded candidate Choice, execution intent Noul calibration, ambiguity detection, readiness Score) and `agent-config` (abstract task profiling across routine, standard, high complexity tiers without vendor model coupling), maintaining deterministic fail-closed safety and zero-dependency offline fallback.
