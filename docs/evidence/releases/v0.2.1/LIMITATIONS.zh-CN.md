...
4. **TypeSafe Jev API Key 依赖说明：** `ask-light`、`agent-config` 与 `project-init` 中的可选语义加速需要配置环境变量 `TYPESAFE_API_KEY` 并安装 `typesafe-sdk`。在未配置或环境不可用时，所有技能自动平滑退回确定性规则基准，功能与安全性不受影响。在非交互式管道中若未显式传入 `--jev`，`project-init` 默认保持标准脚手架行为。
