...
| 字段 | 整集合 | 单 Skill（`kanban-worker`） | 单 Skill（`kb-init`） |
| --- | --- | --- |
...
| 安装包 smoke | — | 安装的 `kanban-worker` 与 tag 源码逐字节一致且套件通过 | 安装的 `kb-init` 与 tag 源码逐字节一致且 contract 测试 OK |
| 重复安装 | 同命令 exit 0；no-op overwrite | 同命令 exit 0；no-op overwrite | 同命令 exit 0；no-op overwrite |
