...
- 稳定版本边界：v0.1.5 包含八个包，不包含 `kb-init`
- 准入状态：完整准入路径 `PASS`；当前默认分支未发布
...
| 结构 | PASS | `SKILL.md`、`agents/openai.yaml`、四份参考文档、`evals/evals.json` 与 contract 测试均存在且非空；contract 测试运行 `OK`。 |
...
本地源码与结构证据属于准入证据，不等于已验证的 release 安装命令。`kb-init` 的安装命令必须等下一个已发布 tag 及 fresh released-repository 验证后再给出。
