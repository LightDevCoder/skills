...
- Stable-release boundary: v0.1.5 contains eight packages and does not contain `kb-init`
- Admission status: `PASS` under the full admission path; unreleased on the current default branch
...
| Structure | PASS | Package `SKILL.md`, `agents/openai.yaml`, four reference documents, `evals/evals.json`, and a contract test are present and non-empty; contract test runs `OK`. |
...
Local-source and structural evidence is admission evidence, not proof of a
released install command. A pinned or generic `kb-init` install command must
wait for the next published tag and fresh released-repository verification.
