...
> **Rename note:** this Skill was named `light-kanban-worker` when first
> released in v0.1.4 and renamed `kanban-worker` in v0.1.6. Historical
> records and pinned `--skill light-kanban-worker` commands in v0.1.4/v0.1.5
> evidence keep the old name.

## What it solves

`kanban-worker` turns a scheduled agent wake-up into exactly one handled
