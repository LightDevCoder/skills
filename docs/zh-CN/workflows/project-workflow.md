...
| --- | --- | --- | --- | --- |
| 1 | 新项目需要最小可确认起点 | [`project-init`](../../../skills/project-init/SKILL.md) — user-invoked | 已验证最小 preset + 初始化路径 | 停止，由用户选下一步 |
...
- 不知入口 → [`ask-light`](../../../skills/ask-light/SKILL.md) `next` — 一个推荐后停止。
- 文稿 / 知识库 / 看板 / 学习 → [specialized-workflows](specialized-workflows.md)。

组合是显式的：路由只推荐，执行由用户显式调用。`SKILL.md` 始终是契约。
