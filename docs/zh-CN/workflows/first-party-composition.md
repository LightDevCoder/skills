...
可能选择 `project-init`、`learn-anything`、`manuscript-ops` 或 `review-loop`。`ask-light` 在建议后停止，不执行、安装、delegate、创建 workflow state 或静默串联 user-invoked Skill；选中的 Skill 保留自己的 contract 和 evidence boundary。
