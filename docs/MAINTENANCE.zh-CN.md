...
准入集合包含 `skills/` 下 **35 个第一方 Skill**（见 [CATALOG.zh-CN.md](../CATALOG.zh-CN.md)）。当前稳定版本为 `v0.2.0`，其发布线现承载全部 34 个第一方包——原始发布 33 个，另加 `humanizer` 准入（见 [CHANGELOG.zh-CN.md](../CHANGELOG.zh-CN.md)）；上一稳定版本为 9 个包的 `v0.1.6`。
