...
本分支的准入集合必须恰好包含 `review-loop`、`project-init`、`ask-light`、`learn-anything`、`manuscript-ops`、`recap` 和 `language-learning`。`recap` 与 `language-learning` 经纯提示型快速通道准入，并在 v0.1.2 中发布；稳定 v0.1.1 包含原来的五个已准入包。`tests/test_collection_discovery.py` 负责检查包名、metadata、目录、README 链接、头图、治理路径、双语配对和退休 orchestration 边界，但不证明 runtime 或 fresh installation。
