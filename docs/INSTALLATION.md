...
The public first-party collection's current stable release is [v0.2.2](https://github.com/LightDevCoder/skills/releases/tag/v0.2.2), published at tag `v0.2.2` (exact commit recorded in the [release receipt](evidence/releases/v0.2.2/RELEASE_RECEIPT.md)). It provides **36 admitted first-party Skills** across 7 purpose-based categories. Package contracts remain inside `skills/<category>/<name>/`; this document is the installation authority and does not replace host-specific discovery rules.
...
Previous releases (such as `#v0.2.1` and `#v0.2.0`) remain available for reproducible historical installs:
